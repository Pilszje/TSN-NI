...
module ptp_parser (
  input        clk, rst,
  input [31:0] int_data,  // packet data
  input        int_valid, // data is valid?
  input        int_sop,   // start of packet
  input        int_eop,   // end of packet
  // input [ 1:0] int_mod,   // mod. Not used apparently
...
always @(posedge clk) begin
  if (rst) begin
    ...
    if (int_valid && int_sop)
      ...
      int_cnt <= int_cnt + 10'd1 - {9'b0,bypass_vlan} - {9'b0,bypass_mpls} - {9'b0,(bypass_ipv4 || bypass_ipv6 || bypass_udp)};
...
always @(posedge clk) begin
  if (rst) begin
    ...
    // bypass vlan
    if      (int_valid && int_cnt==e_type_idx && (int_data[31:16]==c_vlan_tpid_1 || int_data[31:16]==c_vlan_tpid_2 || int_data[31:16]==c_vlan_tpid_3))  // ether_type == vlan
      bypass_vlan <= 1'b1;
    else if (int_valid && int_cnt==e_type_idx+1 && (int_data[31:16]==c_vlan_tpid_1 || int_data[31:16]==c_vlan_tpid_2 || int_data[31:16]==c_vlan_tpid_3) && bypass_vlan)  // vlan_type == vlan
      bypass_vlan <= 1'b1;
    else if (int_valid && bypass_vlan)
      bypass_vlan <= 1'b0;

    // bypass mpls
    if      (int_valid && (int_cnt==e_type_idx || bypass_vlan && int_cnt==e_type_idx+1) && 
            (int_data[31:16]==c_mpls_type_1 || int_data[31:16]==c_mpls_type_2))  // ether_type == mpls
      bypass_mpls <= 1'b1;
    else if (int_valid &&  int_cnt==e_type_idx+1 && bypass_mpls && 
    ...
    // bypass ipv4
    if      (int_valid && (int_cnt==e_type_idx || (bypass_vlan || bypass_mpls) && int_cnt==e_type_idx+1) && bypass_ipv4_cnt==10'd0 &&
    ...
    // bypass ipv6
    if      (int_valid && (int_cnt==e_type_idx || (bypass_vlan || bypass_mpls) && int_cnt==e_type_idx+1) && bypass_ipv6_cnt==10'd0 &&
    ...
    if (int_valid && (int_cnt==e_type_idx || bypass_vlan && int_cnt==e_type_idx+1) && int_data[31:16]==c_ptp2_type)  // ether_type == ptp
      ptp_l2 <= 1'b1;
    ...
    if      (int_valid && (int_cnt==e_type_idx || (bypass_vlan && int_cnt==e_type_idx+1)) && int_data[31:16]==c_ptp2_type &&
            (ptp_msgid_mask[int_data[11: 8]]))  // ptp_message_id == ptp_event
      ptp_event <= 1'b1;
    else if (int_valid && int_cnt==e_type_idx+1 && bypass_udp_cnt==10'd1 && ptp_l4 &&
            (ptp_msgid_mask[int_data[11: 8]]))  // ptp_message_id == ptp_event 
      ptp_event <= 1'b1;
  end
end
...
always @(posedge clk) begin
  if (rst) begin
    ...
    if (int_valid && ptp_cnt==10'd6)
      ptp_cksum <= ptp_cksum + {4'b0, ptp_data[31:24]} + {4'b0, ptp_data[23:16]} + {4'b0, ptp_data[15: 8]} + {4'b0, ptp_data[ 7: 0]};
    if (int_valid && ptp_cnt==10'd7)
      ptp_cksum <= ptp_cksum + {4'b0, ptp_data[31:24]} + {4'b0, ptp_data[23:16]} + {4'b0, ptp_data[15: 8]} + {4'b0, ptp_data[ 7: 0]};
    if (int_valid && ptp_cnt==10'd8)
      ptp_cksum <= ptp_cksum + {4'b0, ptp_data[31:24]} + {4'b0, ptp_data[23:16]};
  end
end
